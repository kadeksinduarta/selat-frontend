                    ...
                    {filteredOrders.length === 0 && (
                        <div className="text-center py-12">
                            ...
                            <p className="text-gray-500 text-lg">
                                {statusFilter === "all" ? "Belum ada pesanan" : `Tidak ada pesanan dengan status ${statusFilter}`}
                            </p>
                        </div>
                    )}
                </div>
                        ...
                        <div className="bg-white rounded-xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
                            <div className="p-6 border-b">
                                <div className="flex items-center justify-between">
                                    <h2 className="text-2xl font-bold text-gray-800">Detail Pesanan #{selectedOrder.id}</h2>
                                    <button
                            ...
                            <div className="p-6 space-y-6">
                                {/* Customer Info */}
                                <div>
                                    <h3 className="text-lg font-semibold text-gray-800 mb-3">Informasi Customer</h3>
                                    <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                                        <p className="text-sm"><span className="font-semibold">Nama:</span> {selectedOrder.user?.name || "-"}</p>
                                        <p className="text-sm"><span className="font-semibold">Email:</span> {selectedOrder.user?.email || "-"}</p>
                                        <p className="text-sm"><span className="font-semibold">Telepon:</span> {selectedOrder.user?.phone || "-"}</p>
                                    </div>
                                </div>

                                {/* Order Items */}
                                <div>
                                    <h3 className="text-lg font-semibold text-gray-800 mb-3">Item Pesanan</h3>
                                    <div className="space-y-3">
                                        {selectedOrder.items?.map((item) => (
                                            <div key={item.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                                                <div className="flex-1">
                                                    <p className="font-medium text-gray-800">{item.product?.name || "Produk"}</p>
                                                    <p className="text-sm text-gray-600">{item.qty} x {formatCurrency(item.price)}</p>
                                                </div>
                                                <p className="font-semibold text-gray-800">{formatCurrency(item.qty * item.price)}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
